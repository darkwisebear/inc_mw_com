...
async fn get_samples<'a, S: Stream<Item=proxy_bridge_rs::SamplePtr<'a, bigdata_gen_rs::MapApiLanesStamped>> + 'a>(map_api_lanes_stamped: S, count: usize) {
...
fn main() {
    println!("[Rust] Size of MapApiLanesStamped: {}", std::mem::size_of::<bigdata_gen_rs::MapApiLanesStamped>());
    println!("[Rust] Size of MapApiLanesStamped::lane_boundaries: {}", std::mem::size_of_val(&bigdata_gen_rs::MapApiLanesStamped::default().lane_boundaries));
    proxy_bridge_rs::initialize(Some(Path::new("./platform/aas/mw/com/test/bigdata/mw_com_config.json")));
    ...
    let bigdata_gen_rs::BigDataProxy { map_api_lanes_stamped } = bigdata_gen_rs::BigDataProxy::new(&handles[0])
